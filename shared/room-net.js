/* ============================================================
   Game Show Central — shared connection-resilience layer.
   VERBATIM COPY of games/jeopardy/js/buzzer-net.js (field-tested;
   see games/jeopardy/docs/buzzer-spec.md §9). Only three things
   changed, all documented in docs/reports/shell-implementation.md:
     1. the UMD export is `RoomNet` instead of `BuzzerNet`;
     2. brokerLabel() says "room server" instead of "buzzer server"
        (the hub hosts four games, not just buzzers) — text only;
     3. one console.warn prefix.
   No logic, no constant and no timing value differs. Do not
   re-derive these numbers; keep this file in step with upstream.
...
(function (root, factory) {
  ...
  function brokerLabel(status) {
    if (status === "reconnecting") return "Reconnecting to the room server… connected players stay in.";
    if (status === "lost") return "Lost the room server — connected players keep working; new joins are paused.";
    return "";
  }
    ...
    function onDisconnected() {
      ...
      timer = setTimer(() => {
        ...
        try {
          ...
          if (typeof console !== "undefined") console.warn("Room broker reconnect failed:", err);
        }
      }, delay);
    }
    function recovered() {
