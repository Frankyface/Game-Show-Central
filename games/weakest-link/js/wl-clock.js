/* ============================================================
   Weakest Link — round-clock renderer (DOM only)
   The pure core stores the clock as {running, deadline,
...
(function (root) {
  const DANGER_MS = 10000; // last ten seconds turn red and tick
  // rAF stops in a background tab (and in some headless panes), which would
  // leave the clock frozen and the expiry never reported. A slow interval
  // paints as a safety net; the expiry latch keeps onExpire to exactly once.
  const SAFETY_MS = 250;
  ...
  /**
   * @param {{el:HTMLElement, getClock:() => ({running:boolean,deadline:number|null,remainingMs:number}|null),
  ...
  function create(options) {
    ...
    let frame = null;
    let safety = null;
    let firedFor = null;   // the deadline we already reported as expired
  ...
  root.WlClock = { create, format, DANGER_MS };
