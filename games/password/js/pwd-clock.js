/* ============================================================
   Password — clock renderer (DOM only)
   The pure core stores the Lightning Round clock as
   {running, deadline, remainingMs} and never runs a timer. This
   file is the only place a frame loop lives: it reads the current
   clock through an injected getter, paints mm:ss, and fires
   `onExpire` EXACTLY ONCE per running period. No state is
   written here.

   Copied in shape from games/pyramid/js/pyr-clock.js, which is
   field-tested: rAF for smoothness plus a slow interval so a
...
(function (root) {
  ...
  root.PwdClock = { create, format, DANGER_MS };
