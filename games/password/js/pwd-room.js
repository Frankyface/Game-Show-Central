/* ============================================================
   ...
   ONE view it is allowed to see (PwdCore.phoneView).

   This is the file that keeps the game honest. Only the two
   givers' views ever carry the password, so a receiver who opens
   their phone's dev tools still learns nothing. The host stays
   authoritative: a giver may tap "Clue given" (and, in the
   Lightning Round, Got it / Pass), and even that is re-checked
   against the state before it becomes an event. Judging a guess
   and calling a clue illegal are host-only and have no phone
   message at all.
   ============================================================ */
...
(function () {
  ...
  function onPlayerJoin(player) {
    ...
    if (app.core) {
      ...
      if (!seated) window.PwdApp.error(`${player.name} joined — they can play from the next game.`);
    } else if (!app.setup.players.some((p) => p.pid === player.pid)) {
      window.PwdApp.addPlayer(player.name, player.pid, false);
    }
    countPhones();
    pushViews(window.PwdApp.core());
  }
  ...
  function onPlayerStatus(pid, connected) {
    delete lastSent[pid];
    countPhones();
    if (connected) pushViews(window.PwdApp.core());
  }

  function onPlayerLeave(pid) {
    delete lastSent[pid];
    if (!window.PwdApp.core()) window.PwdApp.removePlayer(pid);
    countPhones();
  }

  function countPhones() {
    if (!room || !room.players) return;
    window.PwdApp.setPhoneCount(room.players().filter((p) => !p.manual && p.connected !== false).length);
  }

  /* ============ Inbound: intents only, each re-checked ============ */

  function onMessage(pid, raw) {
    const msg = core().validatePhoneMsg(raw);
    if (!msg) return;                       // junk frame: ignored, never thrown
    if (msg.t === "ready") return;          // the push is driven by state, not by asking
    // PwdApp.phoneIntent re-checks that this pid really is the current giver
    // before anything reaches the reducer, and the reducer checks the phase again.
    window.PwdApp.phoneIntent(pid, msg.t);
  }
  ...
  function reportProgress(state) {
    ...
    if (room.reportScores && (state.phase === "result" || state.phase === "standings")) {
      // Lightning Round money goes to BOTH members of the winning team
      // (spec 13 §1); main-game points are shown but never banked.
      const rows = [];
  ...
  function titleFor(state) {
    if (state.phase === "word" && state.round) {
      return `${state.teams[state.round.turn].name} to clue`;
    }
    if (state.phase === "gameOver") return "Game over";
    if (state.phase === "lightning") return "Lightning Round";
  ...
  function wireRoomButton() {
    ...
    btn.addEventListener("click", () => {
      ...
      try {
        ...
        setText("pwd-room-status", `Could not open a room: ${err.message}`);
      }
      paintRoom();
    });
  }
  ...
  async function boot() {
    ...
    try {
      ...
      window.PwdApp.error(`Phones are unavailable: ${err.message}. The host can still run the game.`);
      return;
    }
    ...
    window.PwdApp.bindRoom(room.code);
    if (room.onStatus) {
      room.onStatus(() => { window.PwdApp.bindRoom(room.code); paintRoom(); });
    }
    window.PwdApp.subscribe((state) => { pushViews(state); reportProgress(state); paintRoom(); });
    wireRoomButton();
    ...
    window.PwdRoom = { room, pushViews, onMessage, phoneCount: () => (room.players ? room.players().length : 0) };
  }
